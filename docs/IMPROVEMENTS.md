...
- ~~**IMP-CQ-A08**~~ **Done (2026-08-10)** — `step_host.rs` (`StepHost` / `DirectHost`) was dead-on-arrival: zero callers at any commit since the engine port (`git log -S 'run_script('` matched only the introducing commit `3814f45`), never marked reserved, `pub` in a lib crate so `dead_code` could never fire, and its buffered non-streaming `ScriptOutput` could not serve the live cancel/timeout-aware loop (`handlers/script.rs` + `step_timeout`) without rework. Deleted (file + `pub mod step_host;` + the module-README row), mirroring the `IMP-CQ-008`/`docker_cache.rs` precedent (`b738649`).
...
- ~~**IMP-DO-012**~~ **Done (2026-07-12)** — `register --token` help doesn't warn the token is single-use. Shipped differently than the sketch: the `--token` bearer is a PAT/App token (not itself single-use) — `register --help` now documents the full resolution order on `--token` and the single-use JIT registration warning in the command's long help. Refs: `crates/toolu-runner/src/cli.rs`
