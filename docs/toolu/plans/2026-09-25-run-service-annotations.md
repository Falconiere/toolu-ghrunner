...
- The live workflow jobs require the repository variable `TOOLU_ANNOTATION_82_LIVE=true`. With zero registered runners, their default skipped state keeps the normal PR CI check rollup finite; the skipped jobs are explicitly unverified and the live verifier still fails until a provisioned paired run succeeds.
