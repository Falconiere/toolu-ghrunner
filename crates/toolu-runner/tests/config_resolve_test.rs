...
fn run_git(cwd: &Path, args: &[&str]) -> Result<(), std::io::Error> {
  let output = std::process::Command::new("git")
    ...
    // against the REAL repository instead of the temp one (`git remote add
    // origin` there fails with "remote origin already exists", and would have
    // mutated it had it not). Clear them so `-C` is authoritative.
    .env_remove("GIT_DIR")
