...
_check_file_size() {
  ...
  done < <(find "$_project_root/crates" -name '*.rs' -not -path '*/target/*')
  return $fail
}

_check_no_allow() {
  # Reject #[allow(..)] / #[expect(..)] — must fix the lint, not silence it.
  if grep -RnE '#\[(allow|expect)\(' "$_project_root/crates" --include='*.rs' 2>/dev/null; then
...
_check_no_unwrap() {
  ...
  if grep -RnE '\.(unwrap|expect)\(' "$_project_root/crates" --include='*.rs' 2>/dev/null \
...
_check_no_yamless() {
  ...
  if grep -RnE 'yamless|YAMLESS_' "$_project_root/crates" "$_project_root/Cargo.toml" \
